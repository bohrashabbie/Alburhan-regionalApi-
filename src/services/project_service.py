...
async def get_all_projects(db: AsyncSession, pagination: PaginationParams = None) -> ApiResult:
    try:
        logger.info(f"Fetching projects (page={pagination.page if pagination else 'all'})")
        ...
        if pagination:
            ...
            paginated = PaginatedResponse.create(
                items=[ProjectResponse.model_validate(r) for r in records],
                total=total,
